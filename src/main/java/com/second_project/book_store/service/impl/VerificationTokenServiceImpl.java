...
public class VerificationTokenServiceImpl implements VerificationTokenService {
    ...
    public VerificationToken findByToken(String token) {
        return verificationTokenRepository.findByToken(token)
                .orElseThrow(() -> new RuntimeException("Verification token not found: " + token));
    }

    @Override
    public VerificationToken findByUser(User user) {
        return verificationTokenRepository.findByUserUserId(user.getUserId())
                .orElseThrow(() -> new RuntimeException("Verification token not found for user: " + user.getUserId()));
    }
